...
PRECEDENCE = {
    # Monadic ops
    '|u':7, 'abs':7,'sign':7,'recip':7,'⌊':7,'floor':7,'⌈':7,'ceil':7,'⍟':7,'ln':7,'e':7,'exp':7,'⍳':7,'iota':7,
    # Exponentiation
    '^':6,'**':6,
    # Multiplicative
    '×':5,'*':5,'x':5,'÷':5,'/':5,'%':5,'mod':5,
...
# Dyadic operator mapping\BINARY_OPS = {
    '+': plus, '-': minus,
    '×': times, '*': times, 'x': times,
    '÷': divide, '/': divide,
    '^': power, '**': power,
...
# Utility regex for valid variable names
...
def shunting_yard(tokens):
    """Convert infix tokens to RPN using precedence and parentheses."""
    output, stack = [], []
    for tok in tokens:
...
def eval_rpn(rpn):
    """Evaluate an RPN expression, handling numbers, vars, and ops."""
    stack = []
    for tok in rpn:
        if NUM_RE.match(tok):
...
def eval_expr(expr: str):
    """Evaluate a full expression string to a result."""
